...
let rlMsg = `${greeting}, Saya, ${fUserName.trim()} dari Web TerraPulse, ingin lebih mengetahui terkait Eco-Friendly dan update berita atau informasi terbaru, terima kasih. 🙏`;
  window.open(`https://wa.me/081996275318?text=${rlMsg}`);
}
  ...
  tabButtons.forEach(button => {
    button.addEventListener('click', () => {
      ...
      if (targetContent) {
        ...
        // Load AQI data when AQI tab is activated
...
// ============================================
// AQI DATA FETCHING AND DISPLAY - UPDATED FOR IQAIR API
// ============================================
...
async function loadAQIData() {
  try {
    // IQAir API endpoint for Jakarta
    ...
    if (data.status === "success") {
      // Extract AQI value (US AQI based on PM2.5)
      ...
      // Update weather details from IQAir
      ...
      // Pollutants - IQAir primarily provides PM2.5 based AQI
      const pollution = data.data.current.pollution;
      
      // Update time - IQAir provides timestamp in ISO format
      if (pollution.ts) {
...
// ============================================
// ECO HERO GAMIFICATION SYSTEM - SIMPLIFIED (MAX 300 POINTS)
// ============================================
